...
def train(agent, control_vehicle, episode, target_lane):
    ...
    if cur_ego_info_dict['LaneID'] != '':
        if target_lane == 0:
            r_tl = -(0.0005 * (pre_ego_info_dict['position'][0] - 1100) ) * abs(int(cur_ego_info_dict['LaneID'][-1]) - 0) *1/4
        elif target_lane == 1:
            if int(cur_ego_info_dict['LaneID'][-1]) == 4 or int(cur_ego_info_dict['LaneID'][-1]) == 0:
                r_tl = -(0.0005 * (pre_ego_info_dict['position'][0] - 1100) )
            else:
                r_tl = 0
        else:
...
def main_train():
    ...
    os.mkdir("result_record_pdqn_3r_5tl_tlr_noctrl_linear_5l_test")
    if not TRAIN:
        if gui:
            agent.load_state_dict(torch.load('./0324/result_record_pdqn_3r_5_check/net_params.pth', map_location=torch.device('cpu')))
        else:
            agent.load_state_dict(torch.load('./0417/result_record_pdqn_3r_5tl_tlr_noctrl_linear_5l/net_params.pth'))
    ...
    for epo in range(20000): # 测试时可以调小epo回合次数 
        ...
        print("++++++++++++++++++++++++++++++++++++++++++++++++")
        print(f"++++++++++++++++++++{epo}+++++++++++++++++++++++++")
        print("++++++++++++++++++ result_record_pdqn_3r_5tl_tlr_noctrl_linear_5l_test +++++++++++++++++++++++")
...
#            all_vehicle, rel_up, _ = get_all(control_vehicle, 200) # 前后200m距离
        ...
        # 保存
        df_record.to_csv(f"result_record_pdqn_3r_5tl_tlr_noctrl_linear_5l_test/df_record_epo_{epo}.csv", index = False)
        torch.save(agent.state_dict(), './result_record_pdqn_3r_5tl_tlr_noctrl_linear_5l_test/net_params.pth') 
        pd.DataFrame(data=losses_actor).to_csv('./result_record_pdqn_3r_5tl_tlr_noctrl_linear_5l_test/losses.csv')
