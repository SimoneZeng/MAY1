...
with open("sumo/route_CL2_high_split.rou.xml", "w") as routes:
    # 配置所有的车辆属性和所有的路线
    print("""<routes>
    ...
    <route id="r1" edges="EA EC" />
    <route id="r2" edges="EA ED" />
    <route id="r3" edges="EA EB" />
    """, file=routes)
    ...
    for i in range(timestep):
        ...
        if i % lane_freq == 0: # 0车道在每lane_freq个时间步的第1个时间步发车
            v0_cnt += 1
            print('    <vehicle id="0_%i" type="CarB" route="r%d" depart="%f" departLane="0" departSpeed="20" color="248,248,255"/>' 
                  % (v0_cnt, random.randint(1, 3), 0.5 * i), file=routes)
        # if i % lane_freq == 2: # 1车道在每lane_freq个时间步的第3个时间步发车
        if i % lane_freq == 1:
            v1_cnt += 1
            print('    <vehicle id="1_%i" type="CarB" route="r%d" depart="%f" departLane="1" departSpeed="20" color="248,248,255"/>' 
                  % (v1_cnt, random.randint(1, 3), 0.5 * i), file=routes)
        # if i % lane_freq == 4: # 3车道在每lane_freq个时间步的第5个时间步发车
        if i % lane_freq == 2:
            v2_cnt += 1
            print('    <vehicle id="2_%i" type="CarB" route="r%d" depart="%f" departLane="2" departSpeed="20" color="248,248,255"/>' 
                  % (v2_cnt, random.randint(1, 3), 0.5 * i), file=routes)
        # if i % lane_freq == 6: # 3车道在每lane_freq个时间步的第7个时间步发车
        if i % lane_freq == 3:
            v3_cnt = v3_cnt + 1
            print('    <vehicle id="3_%i" type="CarB" route="r%d" depart="%f" departLane="3" departSpeed="20" color="248,248,255"/>' 
                  % (v3_cnt, random.randint(1, 3), 0.5 * i), file=routes)
        # if i % lane_freq == 8: # 4车道在每lane_freq个时间步的第9个时间步发车
        # if i % lane_freq == 4:
        if i % lane_freq == 0:
            v4_cnt = v4_cnt + 1
            print('    <vehicle id="4_%i" type="CarB" route="r%d" depart="%f" departLane="4" departSpeed="20" color="248,248,255"/>' 
                  % (v4_cnt, random.randint(1, 3), 0.5 * i), file=routes)
