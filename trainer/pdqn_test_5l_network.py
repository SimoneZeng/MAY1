...
def train(agent, control_vehicle, episode, target_dirs):
    '''
    ...
    :return: collision 
    '''
    RoadID = traci.vehicle.getRoadID(control_vehicle)
    print()
    print(f"+++++++++++++++ epo: {episode}  ROAD_ID: {RoadID} ++++++++++++++++")
    print(f"++++++++++++++ {OUT_DIR} ++++++++++++++++")
    global df_record
    stage = RoadID # 当前train是在哪个CL的stage
    ...
    if change_lane=='left':
        if '0' in pre_ego_info_dict["LaneID"]:
            traci.vehicle.moveTo(control_vehicle, traci.vehicle.getRoadID(control_vehicle)+'_1', traci.vehicle.getLanePosition(control_vehicle))
        elif '1' in pre_ego_info_dict["LaneID"]:
            traci.vehicle.moveTo(control_vehicle, traci.vehicle.getRoadID(control_vehicle)+'_2', traci.vehicle.getLanePosition(control_vehicle))
        elif '2' in pre_ego_info_dict["LaneID"]:
            traci.vehicle.moveTo(control_vehicle, traci.vehicle.getRoadID(control_vehicle)+'_3', traci.vehicle.getLanePosition(control_vehicle))
        elif '3' in pre_ego_info_dict["LaneID"]:
            traci.vehicle.moveTo(control_vehicle, traci.vehicle.getRoadID(control_vehicle)+'_4', traci.vehicle.getLanePosition(control_vehicle))
    elif change_lane=='right':
        if '1' in pre_ego_info_dict["LaneID"]:
            traci.vehicle.moveTo(control_vehicle, traci.vehicle.getRoadID(control_vehicle)+'_0', traci.vehicle.getLanePosition(control_vehicle))
        elif '2' in pre_ego_info_dict["LaneID"]:
            traci.vehicle.moveTo(control_vehicle, traci.vehicle.getRoadID(control_vehicle)+'_1', traci.vehicle.getLanePosition(control_vehicle))
        elif '3' in pre_ego_info_dict["LaneID"]:
            traci.vehicle.moveTo(control_vehicle, traci.vehicle.getRoadID(control_vehicle)+'_2', traci.vehicle.getLanePosition(control_vehicle))
...
def main_train():
    ...
    for out_dir in OUT_DIRs:
        ...
        globals()['RL_CONTROL']=200
        globals()['OUT_DIR']=f"{out_dir}/network_test" 
        ...
        for epo in range(episode_num): 
            if re.search(r"lstm", out_dir):
                agent.init_hidden()
            truncated = False # 撞车
            target_dirs_init = None # 初始的target_dir，目标转向方向
            
            # (3) 根据不同的CL_Stage启动对应的sumoCmd
            # stage 1 在5车道中模拟2车道，之后的stage都是5车道
            cfg_path = f"{sumo_dir}cfg_high_network.sumocfg"
            sumoCmd = [sumoBinary, "-c", cfg_path, "--log", f"{OUT_DIR}/logfile.txt"]
            traci.start(sumoCmd)
            ...
            target_dirs_init = [random.randint(0, 2) for _ in range(5)] # ego的变道方向，从0 1 2中取
            ...
            while traci.simulation.getMinExpectedNumber() > 0:
                ...
                if traci.vehicle.getPosition(control_vehicle)[0] < RL_CONTROL:
                    ...
                    continue
                # 3.2 在大于3100m
                if traci.vehicle.getPosition(control_vehicle)[0] >= 5000:
                    print("=======================距离超过3100====================")
                    break
                ...
                # 5 模型训练
                collision, loss_actor, _ = train(agent, control_vehicle, epo,  target_dirs_init) # 模拟一个时间步
                if collision:
                    truncated = True
                    break
