...
from model.d3qn_model_5l_linear import DQNAgent
...
OUT_DIR="result_d3qn_5l_linear_mp"
...
def train(worker, lock, traj_q, agent_q, control_vehicle, episode, target_dir, CL_Stage):
    ...
    if TRAIN:
        action_int = worker.choose_action(np.array(all_vehicle), tl_code) # 离散lane change ，连续acc，参数
    else:
        action_int = worker.choose_action(np.array(all_vehicle), tl_code, train = False)
    ...
    if 'EA_0' == pre_ego_info_dict["LaneID"] and change_lane=='right':
        ...
        traj_q.put((deepcopy(all_vehicle), deepcopy(tl_code), deepcopy(action_int), deepcopy(acc_param),
    ...
    # 计算速度，没有限速控制
    sp = traci.vehicle.getSpeed(control_vehicle) + acc_param*0.5 # 0.5s simulate一次
...
def main_train():
    ...
    worker = DQNAgent(
        N_STATES=agent_param["s_dim"], 
        N_TL=agent_param["tl_dim"],
        N_ACTIONS=agent_param["a_dim"],
    ...
    process=list()
    traj_q=Queue(maxsize=agent_param["memory_size"])
    ...
    for epo in range(episode_num): 
        ...
        while traci.simulation.getMinExpectedNumber() > 0:
            ...
            # 5 模型训练
            collision, q_loss = train(worker, lock, traj_q, agent_q, control_vehicle, epo,  target_dir_init, CL_Stage) # 模拟一个时间步
            if collision:
                truncated = True
                break

            if q_loss is not None:
                losses_actor.append(q_loss)
