...
class NSTEPReplayBuffer:
    ...
    def __init__(self, 
        ...
        self.prev_obs=np.zeros([size, n_step-1, obs_dim], dtype=np.float32)
        self.prev_tl_code=np.zeros([size, n_step-1, tl_dim], dtype=np.float32)
        self.prev_acts=np.zeros([size, n_step-1, 1], dtype=np.float32)
        self.prev_acts_param=np.zeros([size, n_step-1, param_dim], dtype=np.float32)
